...
enum {errcode_ok = 0, errcode_cancel = -1, errcode_format = -2, errcode_file = -3, errcode_directory = -4, errcode_clipboardupdate = -5};
...
void wfClipboard_kos::hdrop_paste_internal(char* err_msg, int max_bytes)
{
	...
	tauto_destruct_executor destruct_executor(boost::bind(&wfClipboard_kos::did_set_paste_event, this));
	...
	for (uint32_t i = 0; i < dsc->cItems && errcode_ == errcode_ok; i++)
	{
		const FILEDESCRIPTOR& fgd = dsc->fgd[i];
		...
		if (fgd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
			...
			if (!ret && errcode_ == errcode_ok) {
				errcode_ = errcode_directory;
			}
			continue;
		}
		{
		...
		while (pos < fsize) {
			...
			if (ret != ERROR_SUCCESS) {
				...
				if (errcode_ == errcode_ok) {
					if (ret == ERROR_BAD_DESCRIPTOR_FORMAT) {
						errcode_ = errcode_clipboardupdate;
					} else {
						errcode_ = errcode_file;
					}
				}
			...
			posix_fwrite(file->fp, this->req_fdata, this->req_fsize);
