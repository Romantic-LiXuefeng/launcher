...
static SSIZE_T transport_read_layer(rdpTransport* transport, BYTE* data, size_t bytes)
{
	...
	SSIZE_T read = client->rose_read_layer(client, data, bytes);
	if (read > 0) {
		rdp->inBytes += read;
	}
	return read;
...
int transport_write(rdpTransport* transport, wStream* s)
{
	...
	if (length > 0)
	{
		...
		status = client->rose_write_layer(client, Stream_Pointer(s), length);
		if (status <= 0) {
			status = -1;
			goto out_cleanup;
		}
...
// freerdp-rose-fix
int rose_did_read(freerdp_peer* client)
{
