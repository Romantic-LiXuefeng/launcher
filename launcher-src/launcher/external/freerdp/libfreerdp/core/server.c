...
static BOOL wts_queue_receive_data(rdpPeerChannel* channel, const BYTE* Buffer, UINT32 Length)
{
	...
	if (channel->channelId == 1006) {
		int ii = 0;
		WLog_WARN(TAG, "wts_queue_receive_data[1006] queue->size: %i, Length: %u", channel->queue->size, Length);
	}
                                          ...
                                          ULONG BufferSize, PULONG pBytesRead)
{
	...
	*pBytesRead = messageCtx->length - messageCtx->offset;
	...
	if (messageCtx->channelId == 1006) {
		int ii = 0;
		WLog_WARN(TAG, "FreeRDP_WTSVirtualChannelRead[1006] channel->queue->size: %i, offset: %u, length: %u, [BytesRead: %u BufferSize: %u]", 
			channel->queue->size, messageCtx->offset, messageCtx->length, *pBytesRead, BufferSize);
	}
