...
struct tlive555_using_lock
{
		...
		, scene(scene)
	{
		...
		VALIDATE(ptr->using_count >= 0, nullptr);
...
int start(const std::string& rtsp_url, bool send_rtcp, std::string& httpd_url)
{
	VALIDATE_IN_MAIN_THREAD();
	...
	if (singleton->rtsp == nullptr) {
		...
		SDL_Log("---live555::start, rtsp_url: %s, construct ourRTSPClient fail", rtsp_url.c_str());
	...
	if (singleton->rtsp == nullptr) {
		...
		SDL_Log("---live555::start, rtsp_url: %s, DESCRIBE, SETUP, PLAY maybe fail", rtsp_url.c_str());
		shutdownLive555(scheduler, env, nullptr);
		return nposm;
	}
	if (!singleton->setup_success) {
	...
	SDL_Log("---live555::start, rtsp_url: %s, httpd_url: %s", rtsp_url.c_str(), httpd_url.c_str());
