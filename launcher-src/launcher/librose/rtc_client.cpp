...
void trtc_client::VideoRenderer::OnFrame(const webrtc::VideoFrame& video_frame)
{
    ...
    if (now - last_verbose_ms_ >= 10000) {
		last_verbose_ms_ = now;
		uint32_t elapsed_second = (now - startup_verbose_ms_) / 1000;
		SDL_Log("[rtc_client]#%i, t: %u, %u*%i fps, %ix%i/%ix%i, rotation(%i)", at_, elapsed_second, frame_thread_frames_ / elapsed_second, sample_interval_, buffer->width(), buffer->height(), video_frame.width(), video_frame.height(), video_frame.rotation());
    }
}
...
void trtspcapture::start_rtsp()
{
	...
	const int clones = rtsps_.size() - 1;
	for (int n = 0; n < clones; n ++) {
		h264_codec.id = first_h264_payload_type_ + n + 1;
		h264_codec.SetParam("rtsp", std::string("ch") + str_cast(n + 1));
