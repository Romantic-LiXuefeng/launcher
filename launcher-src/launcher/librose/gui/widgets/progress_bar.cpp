...
tprogress_bar::tprogress_bar(twindow& window, twidget& widget, const std::string& message, int initial_percentage, const boost::function<void (ttrack&, const SDL_Rect&, int, const std::string&)>& _did_draw_bar)
	: tbase_tpl_widget(window, widget)
	, widget_(dynamic_cast<ttrack*>(&widget))
	, message_(message)
	, align_(tgrid::HORIZONTAL_ALIGN_CENTER)
...
void tprogress_bar::set_align(int align)
{
	VALIDATE(align == tgrid::HORIZONTAL_ALIGN_LEFT || align == tgrid::HORIZONTAL_ALIGN_RIGHT || align == tgrid::HORIZONTAL_ALIGN_CENTER, null_str);
	align_ = align;
}
...
*/
	...
	if (percentage_ != PROGRESS_MAX_PERCENTAGE) {
		render_rect(renderer, SDL_Rect{widget_rect.x, widget_rect.y, widget_rect.w * percentage_ / 100, widget_rect.h}, 0xff00ff00);
	} else {
		if (increase_) {
			blue_ += 2;
		} else {
	...
	if (!message_.empty()) {
		...
		if (align_ == tgrid::HORIZONTAL_ALIGN_LEFT) {
			dest_rect.x = widget_rect.x + gap;

		} else if (align_ == tgrid::HORIZONTAL_ALIGN_RIGHT) {
			dest_rect.x = widget_rect.x + widget_rect.w - gap - surf->w;
		}
