...
public:
	...
	void set_did_double_click(const boost::function<void (ttrack&, const tpoint& coordinate)>& did)
	{ did_double_click_ = did; }
	void set_did_right_button_up(const boost::function<void (ttrack&, const tpoint& coordinate)>& did)
	{ did_right_button_up_ = did; }

	SDL_Rect get_draw_rect() const;
	texture& background_texture() { return background_tex_; }
	void set_did_create_background_tex(const boost::function<texture (ttrack&, const SDL_Rect&)>& did)
...
private:
	...
	boost::function<texture (ttrack&, const SDL_Rect&)> did_create_background_tex_;
	bool background_tex_dirty_;

	/** Mouse left double click callback */
	boost::function<void (ttrack&, const tpoint& coordinate)> did_double_click_;

	boost::function<void (ttrack&, const tpoint& coordinate)> did_right_button_up_;
};
