...
bool set_resolution(CVideo& video, const unsigned width, const unsigned height, bool scene)
{
	...
	SDL_Log("set_resolution(%i, %i, %s), original size: %i x %i", width, height, scene? "true": "false", rect.w, rect.h);
/*
...
*/
	...
	if (landscape_size.x < game_config::min_allowed_width() || landscape_size.y < game_config::min_allowed_height()) {
		return false;
	}
