            ...
            if (size != 0) {
                ...
                if (muxer == NULL) {
                    ...
                    if ((flags & MediaCodec::BUFFER_FLAG_SYNCFRAME) != 0) {
                        ALOGD("It is SYNCFRAME, copy configData(%i bytes)", configDataSize);
                    ...
                    // ALOGV("pre didScreenCaptured(%p, %u, %i, %i)", pixelBuf, (uint32_t)size, (int)gVideoWidth, (int)gVideoHeight);
                    didScreenCaptured(pixelBuf, size, gVideoWidth, gVideoHeight, flags, user);
 ...
 * around.
 */
static status_t recordScreen(uint8_t* pixelBuf, fdid_gui2_screen_captured didScreenCaptured, void* user) {
    status_t err;
/*
...
NDK_EXPORT int kosRecordScreenLoop(uint32_t bitrate_kbps, uint8_t* pixel_buf, fdid_gui2_screen_captured did, void* user) {
    ALOGD("screenrecord.cpp::kosRecordScreenLoop---");
    gVerbose = true;
    ...
    status_t err = recordScreen(pixel_buf, did, user);
    gPause = false;
    gRequireSetPause = false;
